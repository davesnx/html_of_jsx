...
JSX.stringf("%s %i", "Ada", 3)    /* format string -> JSX.element */
JSX.int(42)                       /* int           -> JSX.element */
JSX.float(3.14)                   /* float         -> JSX.element */
JSX.null                          /* value that represents nothing */
```
```mlx
JSX.string "Hello"                (* string        -> JSX.element *)
JSX.stringf "%s %i" "Ada" 3       (* format string -> JSX.element *)
...
Use `JSX.stringf` when you want `Printf`\-style formatting such as `%s`, `%c`, `%i`, `%d`, `%f`, or `%%`, while still rendering escaped text.

```reasonml
let profile = (~name, ~visits, ()) => {
  <p> {JSX.stringf("Welcome %s (%i visits)", name, visits)} </p>;
};
```
```mlx
let profile ~name ~visits () =
  <p>(JSX.stringf "Welcome %s (%i visits)" name visits)</p>
```
Just like `JSX.string`, the formatted result is escaped before it is written to HTML.
