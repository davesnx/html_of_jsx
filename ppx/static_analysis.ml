...
type static_part =
  ...
  | Dynamic_format of expression * (arg_label * expression) list
...
let extract_jsx_format_call expr =
  match expr.pexp_desc with
  | Pexp_apply
      ( { pexp_desc = Pexp_ident { txt = Ldot (Lident "JSX", "format"); _ }; _ },
        (Nolabel, fmt) :: args
      )
  | Pexp_apply
      ( { pexp_desc = Pexp_ident { txt = Lident "format"; _ }; _ },
        (Nolabel, fmt) :: args
      ) ->
      Some (fmt, args)
  | _ ->
...
let render_literal_format fmt args =
...
let extract_jsx_format_literal expr =
  match extract_jsx_format_call expr with
  | Some (fmt_expr, args) -> (
      match extract_literal_string fmt_expr with
      | Some fmt ->
          render_literal_format fmt args |> Option.map escape_html
      | None ->
          None
    )
  | None ->
...
let analyze_child (expr : expression) : static_part =
  List.find_map
    ...
    [
      ...
      (fun () ->
        extract_jsx_format_literal expr |> Option.map (fun s -> Static_str s)
      );
      (fun () ->
        extract_jsx_format_call expr
        |> Option.map (fun (fmt, args) -> Dynamic_format (fmt, args))
      );
      (fun () ->
...
let static_size_of_parts parts =
  List.fold_left
    (fun acc part ->
...
let dynamic_count_of_parts parts =
  List.fold_left
    (fun acc part -> match part with Static_str _ -> acc | _ -> acc + 1)
    0 parts

let needs_buffer_of_parts parts =
  let parts = coalesce_static_parts parts in
  Needs_buffer
    {
      parts;
      static_size = static_size_of_parts parts;
      dynamic_count = dynamic_count_of_parts parts;
    }
...
let analyze_element ~tag_name ~attrs ~children =
  ...
  | All_static attrs_html, All_static_children children_html ->
      ...
      in
      Fully_static html
  | All_static attrs_html, (All_string_dynamic parts | Mixed_children parts) ->
