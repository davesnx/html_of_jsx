...
let min_length, max_height =
  ...
  let prev = ref 0 and last = ref 1 and i = ref 0 in
  try
    while !i < d - 1 do
...
let empty = Sub ("", 0, 0)
let length = function Sub (_, _, len) -> len | Concat (_, len, _, _, _) -> len
let height = function Sub (_, _, _) -> 0 | Concat (h, _, _, _, _) -> h
let is_empty = function Sub (_, _, len) -> len = 0 | _ -> false
let is_not_empty = function Sub (_, _, len) -> len <> 0 | _ -> true
...
let print =
  ...
  let map_right = function
    | [] -> []
    | x :: tl -> ("\\" ^ x) :: List.map (fun r -> " " ^ r) tl
  in
  let rec leaves_list = function
    | Sub (s, i0, len) -> [ String.sub s i0 len ]
    | Concat (_, _, l, _, r) ->
        map_left (leaves_list l) @ map_right (leaves_list r)
  in
...
let rec make_of_gen gen ofs len ~n =
  if n <= 1 then if len > 0 then Sub (gen ofs len, 0, len) else empty
  else
    let nl = n / 2 in
...
let rec copy_to_subbytes t ofs = function
  | Sub (s, i0, len) -> Bytes.blit_string s i0 t ofs len
  | Concat (_, _, l, ll, r) ->
...
let to_string = function
  | Sub (s, i0, len) ->
      ...
      if i0 = 0 && len = String.length s then s else String.sub s i0 len
  | r ->
      let len = length r in
...
let rec unsafe_blit src srcofs dst dstofs len =
  match src with
  | Sub (s, i0, _) -> String.blit s (i0 + srcofs) dst dstofs len
  | Concat (_, _, l, ll, r) ->
      let rofs = srcofs - ll in
      if rofs >= 0 then unsafe_blit r rofs dst dstofs len
      else
        let llen = -rofs in
        (* # of chars after [srcofs] in the left rope *)
        if len <= llen then unsafe_blit l srcofs dst dstofs len
        else (
          (* len > llen *)
...
let rec get rope i =
  match rope with
  | Sub (s, i0, len) ->
      if i < 0 || i >= len then raise (Out_of_bounds "Rope.get") else s.[i0 + i]
  | Concat (_, _, l, left_len, r) ->
      if i < left_len then get l i else get r (i - left_len)

let rec iter f = function
  | Sub (s, i0, len) ->
      for i = i0 to i0 + len - 1 do
        f s.[i]
      done
  | Concat (_, _, l, _, r) ->
      iter f l;
...
let balance_concat rope1 rope2 =
  let len1 = length rope1 and len2 = length rope2 in
...
let rec balance_insert forest rope =
  match rope with
  | Sub (s, i0, len) ->
...
let concat_forest forest =
  let concat (n, sum) r =
    ...
    (n + 1, if n = level_flatten then flatten sum else sum)
  in
...
let rec relocate_topright rope leaf len_leaf =
  match rope with
...
let rec relocate_topleft leaf len_leaf rope =
  match rope with
  | Sub (_, _, _) -> raise Relocation_failure
  | Concat (h, len, l, ll, r) ->
...
let concat2_nonempty rope1 rope2 =
    ...
    when h1 > height rope2 -> (
      ...
      if lens <= small_rope_length then (
        ...
        with Relocation_failure ->
          ...
          if
            (h1 = h2plus1 && len2 <= max_flatten_length)
            || len2 < small_rope_length
          then Concat (h1 + 1, len, rope1, len1, flatten rope2)
          else
            (* [h1 > h2 + 1] *)
  ...
  | _, Concat (h2, len2, (Sub (s2, i2, lens2) as leaf2), _, r2)
    when height rope1 < h2 -> (
      ...
      let len = len1 + len2 and lens = len1 + lens2 in
      if lens <= small_rope_length then (
        let s = Bytes.create lens in
        ...
        with Relocation_failure ->
          ...
          (* if replacing [leaf2] will increase the height or if further
             concat will have an opportunity to add to a (small) leaf *)
          if
            (h1plus1 = h2 && len1 <= max_flatten_length)
            || len1 < small_rope_length
          then Concat (h2 + 1, len, flatten rope1, len1, rope2)
          else
            (* [h1 + 1 < h2] *)
  ...
  | _, _ ->
      ...
      if len <= small_rope_length then (
        ...
        let rope1 = if len1 <= small_rope_length then flatten rope1 else rope1
        and rope2 =
          if len2 <= small_rope_length then flatten rope2 else rope2
        in
        let h = 1 + max (height rope1) (height rope2) in
        Concat (h, len1 + len2, rope1, len1, rope2)
...
let rec sub_to_substring flat j i len = function
  | Sub (s, i0, _) -> Bytes.blit_string s (i0 + i) flat j len
  | Concat (_, _, l, ll, r) ->
      let ri = i - ll in
...
let rec trim_left = function
  | Sub (s, i0, len) ->
      let i = ref i0 in
      let i_max = i0 + len in
      while !i < i_max && is_space (String.unsafe_get s !i) do
        incr i
      done;
...
let rec trim_right = function
  | Sub (s, i0, len) ->
      ...
      while !i >= i0 && is_space (String.unsafe_get s !i) do
        decr i
      done;
...
let rec unsafe_index offset i c = function
  | Sub (s, i0, len) -> index_string (offset - i0) s (i0 + i) (i0 + len) c
  | Concat (_, _, l, ll, r) ->
...
let index_from r i c =
  if i < 0 || i >= length r then invalid_arg "Rope.index_from"
  else
...
let rec unsafe_rindex offset i c = function
  | Sub (s, i0, _) -> rindex_string (offset - i0) s i0 (i0 + i) c
  | Concat (_, _, l, ll, r) ->
...
module Iterator = struct
  ...
  let set_current_for_index itr = set_current_for_index_rec itr 0 itr.i itr.rope
  ...
  let make r i0 =
    ...
    in
    if i0 >= 0 && i0 < len then set_current_for_index itr;
    (* force [current] to be set *)
    itr

  let peek itr i =
    if i < 0 || i >= itr.len then raise (Out_of_bounds "Rope.Iterator.peek")
    else if itr.current_g0 <= i && i < itr.current_g1 then
      itr.current.[i + itr.current_offset]
    else get itr.rope i (* rope get *)
...
let compare r1 r2 =
  ...
  let i1 = Iterator.make r1 0 and i2 = Iterator.make r2 0 in
  try
    for _i = 1 to min len1 len2 do
...
let search_forward_string p =
  ...
  fun rope i0 ->
    let i = Iterator.make rope i0 and j = ref 0 in
    (try
       (* The iterator will raise an exception of we go beyond the
          length of the rope. *)
       while !j < m do
         if p.[!j] = Iterator.get i then (
           Iterator.incr i;
           incr j)
         else if !j = 0 then Iterator.incr i
         else j := next.(!j)
       done
...
module Buffer = struct
  ...
  *)
  ...
  let create n =
    let n =
      if n < 1 then small_rope_length
      else if n > Sys.max_string_length then Sys.max_string_length
      else n
    in
    {
      buf = Bytes.create n;
  ...
  let add_char b c =
    if b.length = max_int then
      failwith "Rope.Buffer.add_char: buffer length will exceed the int range";
    if b.pos >= b.buf_len then (
      (* Buffer full, add it to the forest and allocate a new one: *)
      add_nonempty_to_forest b.forest
        (Sub (Bytes.unsafe_to_string b.buf, 0, b.buf_len));
  ...
  let unsafe_add_substring b s ofs len =
    ...
    if b.length > max_int - len then
      failwith
        "Rope.Buffer.add_substring: buffer length will exceed the int range";
  ...
  let add_rope b (r : rope) =
    if is_not_empty r then (
      let len = length r in
      if b.length > max_int - len then
        failwith "Rope.Buffer.add_rope: buffer length will exceed the int range";
      ...
      add_nonempty_to_forest b.forest r;
      (* [r] not empty *)
      b.length <- b.length + len)

  let add_buffer b b2 =
    if b.length > max_int - b2.length then
      failwith "Rope.Buffer.add_buffer: buffer length will exceed the int range";
    add_to_forest b.forest (Sub (Bytes.sub_string b.buf 0 b.pos, 0, b.pos));
    b.pos <- 0;
  ...
  let add_channel b ic len =
    if b.length > max_int - len then
      failwith
        "Rope.Buffer.add_channel: buffer length will exceed the int range";
    let buf_left = b.buf_len - b.pos in
    if len <= buf_left then (
      ...
      while !len >= b.buf_len do
        ...
        add_nonempty_to_forest b.forest
          (Sub (Bytes.unsafe_to_string s, 0, b.buf_len));
        len := !len - b.buf_len
      done;
  ...
  let rec nth_forest forest k i len =
    ...
    let r = forest.(k) in
    (* possibly empty *)
    let ofs = len - length r in
    (* offset of [r] in the full rope *)
    if i >= ofs then get r (i - ofs) else nth_forest forest (k + 1) i ofs
  ...
  let unsafe_sub (b : t) i0 len =
    ...
    if buf_i1 >= len then
      ...
      (if buf_i1 > 0 then (
         ...
         sum :=
           balance_concat !sum
             (Sub (Bytes.sub_string b.buf 0 buf_i1, 0, buf_i1)))
       else
  ...
  let sub b i len =
    if i < 0 || len < 0 || i > b.length - len then invalid_arg "Rope.Buffer.sub";
    if len = 0 then empty else unsafe_sub b i len

  let contents b = if b.length = 0 then empty else unsafe_sub b 0 b.length
...
let print_endline rope =
  output_string stdout rope;
  print_newline ()

let prerr_string rope = output_string stderr rope

let prerr_endline rope =
  output_string stderr rope;
  prerr_newline ()
...
let rec length_leaves = function
  | Sub (_, _, len) -> (len, len)
  | Concat (_, _, l, _, r) ->
      let min1, max1 = length_leaves l and min2, max2 = length_leaves r in
...
let distrib_leaves =
  let rec add_leaves m = function
    | Sub (_, _, len) -> (
        try incr (IMap.find len !m) with _ -> m := IMap.add len (ref 1) !m)
    | Concat (_, _, l, _, r) ->
        add_leaves m l;
        add_leaves m r
  in
